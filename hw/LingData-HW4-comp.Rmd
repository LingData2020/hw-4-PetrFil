...
```{r}
length(unique(mono_socio$ParticipantID)) #1
length(which(grepl("^male", mono_socio$Responce)))
length(which(grepl('female', mono_socio$Responce))) #2
education <- mono_socio$Responce[mono_socio$QuestionCategory == 'education']
unique(education) #3
sum(education == 'College')
sum (education == "UniversityDegree")
sum (education == "Postgraduate")
sum (education == "PhDongoing")  #4
hand <- mono_socio$Responce[mono_socio$QuestionCategory == 'handedness']
sum(hand == "right")
sum(hand == "left") #5
...
No, it doesn't. It has a positive skew(to the left) and looks too sharp.
...
```{r}
ggplot() +
  geom_density(data = mono, aes(RTlog))
...
It looks more like a normal distribution. It is less sharp and has a slightly positive skew.
...
```{r}
meanRTlog <- mean(mono$RTlog)
dev <- sd(mono$RTlog)
mono1 <- 
mono %>% 
filter(RT>600 & abs(RTlog - meanRTlog) < 3 * dev & TypeOfStimuli != "FillerAcceptable" & TypeOfStimuli != "FillerUnacceptable")%>% 
...
ggplot(mono1, aes(x = AcceptabilityJ, y = RTlog, fill = WordOrder)) +   geom_violin(position = pos) + geom_boxplot(width=0.3, position = pos)
...
```{r}
library(ggpubr)
ggline(mono1, x = "AcceptabilityJ", y = "RTlog", add = "mean_se", color = "WordOrder", palette = "simpsons")
...
For each particular participant there is a differene in reaction time  for Congruent and Incongruent stimuli

How can you interpret difference with the result of 2.3?

In 2.3 we compared all Congruent stimuli with all Incongruent stimuli regardless the participants. We analyzed the RT difference of the whole group. In this case, we compared the difference between Congruent and Incongruent stimuli for each particular person. It seems more correct as different people are supposed to demonstrate relatively different results. And we want to compare whether one and the same person has different RT to different stimuli.
...
We use ANOVA test as there are different groups(more than two) and one parameter. The p-value< 0.05 ((Pr(>F))). It means that we should reject Ho. In other words, there is a statistically significant difference in logarithm of reaction time for different types of stimuli. But we don't know between which pairs of groups there is a difference.

```{r}
anova_test <- aov(RTlog ~ TypeOfStimuli, incong_rtlog)
summary(anova_test)
```
...
We can observe that one pair has statistically significant difference between logarithms of reaction time: SubjectiveComment-Material-Shape-Color. SubjectiveComment-Material-Size-Nationality is very close to being significant but p-adj>0.05. The difference between logarithms of reaction time in Size-Nationality-Shape-Color pair is not statistically significant at all.
